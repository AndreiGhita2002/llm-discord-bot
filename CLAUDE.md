...
[ ] Standardise the command set: `voice.py` uses `!play`/`!skip`/… but `discord_logging.py` still uses `/setlogchannel` and `/clearlogchannel`, so the bot answers to two different prefixes. Settle on one (`!` avoids colliding with Discord's slash-command autocomplete popup), move the log commands over, and add a `!help` that lists everything with its arguments. Right now each module matches its own regex and `on_message` calls them in sequence - worth factoring into one shared dispatcher (name -> handler + help text) so adding a command is one entry, the way `tools.py` does it.
