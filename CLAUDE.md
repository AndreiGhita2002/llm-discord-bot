...
**Requirements**: `yt-dlp` + `discord.py[voice]` (which brings `PyNaCl` **and `davey`**) from pip,
and `ffmpeg` + libopus (system; `brew install ffmpeg` covers both on macOS). All are checked in
`configure()` at startup *and* per command, so a missing piece degrades to a clear chat message.
libopus isn't always on the path discord.py autodetects, so `_ensure_opus()` tries the usual
homebrew/linux locations (override with `voice.opus_path`).

**DAVE / E2EE is mandatory** (this bit is easy to get wrong): Discord enforced its end-to-end
encryption protocol for non-stage voice on 2026-03-02. discord.py's `max_dave_protocol_version`
returns 0 when `davey` can't be imported, the bot advertises "no E2EE", and the voice gateway
closes the handshake with **close code 4017** on every attempt - which presents as the bot
joining for a few seconds, playing nothing, and leaving. discord.py supports DAVE from 2.7.0 but
*only* via the `voice` extra, so the dependency must stay `discord.py[voice]`, never plain
`discord.py` + `PyNaCl`. `_HAS_DAVE` in voice.py guards this so it can't regress silently.

Failed connects go through `_connect_failure_detail()`: discord.py raises a bare
`asyncio.TimeoutError` once its internal retries are exhausted, which stringifies to nothing, so
the original 4017 outage logged as `Could not join voice channel 123:` with an empty reason.
...
[x] Voice rejected with close code 4017: Discord's DAVE (E2EE) enforcement. Fixed by depending on `discord.py[voice]>=2.7.0` (pulls `davey`); `_HAS_DAVE` check + `_connect_failure_detail()` make a recurrence self-explanatory instead of silent. See "Voice Playback" above.
[ ] Live-verify voice playback on the deployed bot: ffmpeg + libopus are present on the host (libopus loaded from /opt/homebrew/lib/libopus.dylib in the 2026-08-17 logs) and the Connect/Speak permissions are granted, but no audio has actually played yet - the DAVE fix needs confirming with a real `/play`. Unit + mock-integration tested (queue order, skip, stop, idle disconnect, all `/play` gating paths) and yt-dlp search/stream-URL resolution verified against real YouTube, but no actual audio has been pushed to a real voice channel yet.
